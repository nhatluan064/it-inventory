...
export const useDynamicData = (currentUser, equipment = []) => {
  ...
  const updateCategory = useCallback(
    async (categoryId, categoryData) => {
      ...
      if (isCategoryInUse(categoryId)) {
        toast.error(
          "Không thể cập nhật danh mục đang được sử dụng trong danh sách thiết bị"
        );
        return false;
      }
  ...
  const deleteCategory = useCallback(
    async (categoryId) => {
      ...
      if (isCategoryInUse(categoryId)) {
        toast.error(
          "Không thể xóa danh mục đang được sử dụng trong danh sách thiết bị"
        );
        return false;
      }
      ...
      try {
        ...
        if (!snap.empty) {
          toast.error(
            "Danh mục đang được sử dụng trong thiết bị. Không thể xóa."
          );
          return false;
        }
      } catch (checkErr) {
        ...
        toast.error(
          "Không thể kiểm tra trạng thái sử dụng của danh mục. Thao tác bị hủy."
        );
        return false;
      }
  ...
  const addDepartment = useCallback(
    async (departmentData) => {
      ...
      try {
        // Check for duplicates
        const existing = (departmentsList || []).some(
          (dept) =>
        ...
        await addDoc(
          collection(db, "users", currentUser.uid, "departments"),
          newDepartment
        );
  ...
  const updateDepartment = useCallback(
    async (departmentId, departmentData) => {
      if (!currentUser) return;
  ...
  const deleteDepartment = useCallback(
    async (departmentId) => {
      ...
      try {
        await deleteDoc(
          doc(db, "users", currentUser.uid, "departments", departmentId)
        );
  ...
  const addPosition = useCallback(
    async (positionData) => {
      ...
      try {
        // Check for duplicates
        const existing = (positionsList || []).some(
        ...
        await addDoc(
          collection(db, "users", currentUser.uid, "positions"),
          newPosition
        );
  ...
  const deletePosition = useCallback(
    async (positionId) => {
      ...
      try {
        await deleteDoc(
          doc(db, "users", currentUser.uid, "positions", positionId)
        );
  ...
  const autoAddCategoryIfNotExists = useCallback(
  ...
  const autoAddDepartmentIfNotExists = useCallback(
    async (departmentName) => {
      ...
      if (success) {
        const newDepartment = (departmentsList || []).find(
          (dept) => dept.name.toLowerCase() === departmentName.toLowerCase()
        );
  ...
  const autoAddPositionIfNotExists = useCallback(
    async (positionName) => {
      if (!currentUser || !positionName) return null;

      const existing = (positionsList || []).find(
