...
const DashboardView = ({
  ...
  setActiveTab,
  t,
  // Optional external scroll state; if not provided, use internal to avoid parent re-renders on scroll
  scrollPosition,
  setScrollPosition,
}) => {
  const scrollContainerRef = useRef(null);
  const debounceTimeoutRef = useRef(null);
  const [internalScrollPosition, setInternalScrollPosition] = useState(0);

  const effectiveScrollPosition =
    typeof scrollPosition === "number"
      ? scrollPosition
      : internalScrollPosition;
  const updateScrollPosition =
    typeof setScrollPosition === "function"
      ? setScrollPosition
      : setInternalScrollPosition;

  useLayoutEffect(() => {
    if (scrollContainerRef.current) {
      scrollContainerRef.current.scrollLeft = effectiveScrollPosition;
    }
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      debounceTimeoutRef.current = setTimeout(() => {
        if (scrollContainerRef.current) {
          updateScrollPosition(scrollContainerRef.current.scrollLeft);
        }
      }, 300);
    };

    // Ensure preventDefault works to avoid page scroll/refresh behavior
