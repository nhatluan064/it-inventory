      ...
      <div className="glass-effect bg-gradient-to-br from-white/90 to-gray-50/90 dark:from-gray-800/90 dark:to-gray-900/90 rounded-2xl shadow-2xl border border-gray-100/50 dark:border-gray-700/50 p-6 backdrop-blur-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">{t("search")}</label>
            ...
            <div>
                <label className="block text-xs font-medium mb-2 opacity-0">.</label>
                <button
                    onClick={onOpenAddFromMasterModal}
          ...
          <table className="w-full text-xs">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 border-b-2 border-gray-200 dark:border-gray-700">
                <tr>
                    ...
                    <th className="px-4 py-3.5 text-center font-medium uppercase">{t("purchase_quantity")}</th>
              ...
              {filteredItems && filteredItems.length > 0 ? (
                filteredItems.map((item, index) => (
                  <tr key={item.id} className={`transition-colors duration-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 ${index % 2 === 0 ? 'bg-gray-50/30 dark:bg-gray-900/20' : ''}`}>
                    ...
                    <td className="px-4 py-3.5 capitalize text-gray-600 dark:text-gray-300">{(categories.find((c) => c.id === item.category) || {}).name || item.category}</td>
                    <td className="px-4 py-3.5 text-center">
                      <input type="number" min="1" value={purchaseData[item.id]?.quantity || 1} onChange={(e) => handleDataChange(item.id, "quantity", e.target.value)} className="w-24 p-2 border-2 rounded-xl text-center dark:bg-gray-700 dark:border-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"/>
                    </td>
                    <td className="px-4 py-3.5 text-right">
                      <input type="number" min="0" placeholder="0" value={purchaseData[item.id]?.price || ""} onChange={(e) => handleDataChange(item.id, "price", e.target.value)} className="w-36 p-2 border-2 rounded-xl text-right dark:bg-gray-700 dark:border-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"/>
                    </td>
                    <td className="px-4 py-3.5 text-center">
                      <div className="flex items-center justify-center space-x-2">
                        <button onClick={() => handlePurchaseClick(item)} className="p-2 rounded-lg text-green-500 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors" title={t("purchasing")}><ShoppingCart className="w-4 h-4" /></button>
                        <button onClick={() => onDeleteItem("delete-pending", item)} className="p-2 rounded-lg text-red-500 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors" title={t("delete")}><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))
          ...
          filteredItems.map((item) => (
            <div key={item.id} className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-4 space-y-3">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => onDeleteItem("delete-pending", item)} className="p-2 rounded-xl text-red-500 hover:bg-red-100 dark:hover:bg-red-900/30 -mt-2 -mr-2" title={t("delete")}><Trash2 className="w-4 h-4" /></button>
              </div>
              <div className="flex items-end gap-3 pt-3 border-t dark:border-gray-700">
                <div className="w-1/3">
                  ...
                  <input type="number" min="1" value={purchaseData[item.id]?.quantity || 1} onChange={(e) => handleDataChange(item.id, "quantity", e.target.value)} className="w-full p-2 border-2 rounded-xl text-center dark:bg-gray-700"/>
                </div>
                <div className="flex-grow">
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t("price")} (VNĐ)</label>
                  <input type="number" min="0" placeholder="0" value={purchaseData[item.id]?.price || ""} onChange={(e) => handleDataChange(item.id, "price", e.target.value)} className="w-full p-2 border-2 rounded-xl text-right dark:bg-gray-700"/>
                </div>
                <button onClick={() => handlePurchaseClick(item)} className="p-2.5 rounded-xl text-green-500 bg-green-50 dark:bg-green-900/30 hover:bg-green-100" title={t("purchasing")}><ShoppingCart className="w-5 h-5" /></button>
              </div>
            </div>
          ))
        ) : (
